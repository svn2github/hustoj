...
function read_buf( $buf, $key, &$value )
{
	if ( strncmp($buf,$key,strlen($key)) == 0 )
	{
		$value = substr($buf,equal_pos($buf));
		$value = trim($value);
		return true;
	}
	return false;
...
function read_int($buf, $key, &$value)
{
	if ( read_buf($buf, $key, $tmpbuf) )
	{
		$value = intval($tmpbuf);
		return true;
	}
...
if (isset($_SESSION['OJ_LANG'])) $OJ_LANG=$_SESSION['OJ_LANG'];
	...
	//  for sae.sina.com.cn
		...
		//for normal install
		if(!mysql_pconnect($DB_HOST.":".$DB_PORT,$DB_USER,$DB_PASS)) 
