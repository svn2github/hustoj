        ...
        if ($_REQUEST['action']=='new'){
                if (array_key_exists('title',$_POST) && array_key_exists('content', $_POST) && $_POST['title']!='' && $_POST['content']!=''){
                        ...
                        if(array_key_exists('cid',$_REQUEST)&&$_REQUEST['cid']!='')
                                ...
                                $cid='NULL';
                        $sql="INSERT INTO `topic` (`title`, `author_id`, `cid`, `pid`) SELECT '".mysql_escape_string($_POST['title'])."', '".mysql_escape_string($_SESSION['user_id'])."', $cid, '".mysql_escape_string($pid)."'";
                        if($pid!=0)
                                if($cid!='NULL')
                                        $sql.=" FROM `contest_problem` WHERE `contest_id` = $cid AND `problem_id` = '".mysql_escape_string($pid)."'";
                                else
                                        $sql.=" FROM `problem` WHERE `problem_id` = '".mysql_escape_string($pid)."'";
                        else if($cid!='NULL')
